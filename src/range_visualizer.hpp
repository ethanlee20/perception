...
class Range_Visualizer 
{
    public:
        ...
        Range_Visualizer(OLED_Display *display, float max_measured_radius);
        void initialize();
        void clear();
        void draw_dot_at(float measured_radius, float measured_angle);

    private:
        OLED_Display *display = nullptr;
        float max_measured_radius; // change name
        int max_pixel_radius; // change name
